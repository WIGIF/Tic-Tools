#!/usr/bin/python3

from functools import reduce
from typing import List
from argparse import ArgumentParser
from sys import argv

SBOX_EN = (
    
)

RCON = (
    
)

X2 = (
    
)

X3 = (
    
)

X9 = (
    
)

X11 = (
    
)

X13 = (
    
)

X14 = (
    
)

def print_state(state: bytes) -> None:
    assert len(state) == 16
    for i in range(4):
        for j in range(4):
            print(bytes([state[i + 4*j]]).hex(), end=" ")
        print()

def sub_word(word: bytes) -> bytes:
    """Run word through the sbox"""
    return bytes(SBOX_EN[b] for b in word)

def sub_bytes(state: bytes) -> bytes:
    """SubBytes transformation"""
    assert len(state) == 16
    return sub_word(state)

def sub_bytes_inverse(state: bytes) -> bytes:
    """Invert the sbox"""
    assert len(state) == 16
    return bytes(SBOX_EN.index(b) for b in state)

def rot_word(word : bytes, shift : int = 1) -> bytes:
    """Rotate Word in the matrix"""
    assert len(word) == 4
    shift %= 4
    return word[shift:] + word[:shift]

def shift_rows(state : bytes) -> bytes:
    """ShiftRows transformation"""
    assert len(state) == 16
    matrix = [state[4 * i: 4 * (i + 1)] for i in range(0, 4)] #Create a matrix from the state
    matrix_t = list(map(bytes, zip(*matrix))) #Transpose the matrix
    for i in range(4):
        matrix_t[i] = rot_word(matrix_t[i], i)
    state = b"".join(list(map(bytes, zip(*matrix_t))))
    return state

def shift_rows_inverse(state : bytes) -> bytes:
    """ShiftRows transformation"""
    assert len(state) == 16
    matrix = [state[4 * i: 4 * (i + 1)] for i in range(0, 4)] #Create a matrix from the state
    matrix_t = list(map(bytes, zip(*matrix))) #Transpose the matrix
    for i in range(4):
        matrix_t[i] = rot_word(matrix_t[i], -1*i)
    state = b"".join(list(map(bytes, zip(*matrix_t))))
    return state

def mix_column(a : bytes) -> bytes:
    """Multiply the vector a by the MixColumn matrix over GF(2^8)"""
    result = [
        reduce(fixed_xor, map(to_bytes, [X2[a[0]], X3[a[1]], a[2], a[3]])),
        reduce(fixed_xor, map(to_bytes, [a[0], X2[a[1]], X3[a[2]], a[3]])),
        reduce(fixed_xor, map(to_bytes, [a[0], a[1], X2[a[2]], X3[a[3]]])),
        reduce(fixed_xor, map(to_bytes, [X3[a[0]], a[1], a[2], X2[a[3]]]))
    ]
    return b"".join(result)

def mix_column_inverse(a: bytes) -> bytes:
    """Multiply the vector a by the inverse MixColumn matrix over GF(2^8)"""
    result = [
        reduce(fixed_xor, map(to_bytes, [X14[a[0]], X11[a[1]], X13[a[2]], X9[a[3]]])),
        reduce(fixed_xor, map(to_bytes, [X9[a[0]], X14[a[1]], X11[a[2]], X13[a[3]]])),
        reduce(fixed_xor, map(to_bytes, [X13[a[0]], X9[a[1]], X14[a[2]], X11[a[3]]])),
        reduce(fixed_xor, map(to_bytes, [X11[a[0]], X13[a[1]], X9[a[2]], X14[a[3]]]))
    ]
    return b"".join(result)

def mix_columns(state : bytes) -> bytes:
    """MixColumns transformation"""
    assert len(state) == 16
    matrix = [state[4 * i: 4 * (i + 1)] for i in range(0, 4)]
    state = b""
    for column in matrix:
        state += mix_column(column)
    return state

def mix_columns_inverse(state : bytes) -> bytes:
    """MixColumns transformation"""
    assert len(state) == 16
    matrix = [state[4 * i: 4 * (i + 1)] for i in range(0, 4)]
    state = b""
    for column in matrix:
        state += mix_column_inverse(column)
    return state

def add_round_key(key : bytes, state : bytes) -> bytes:
    """Add a given round key to the state"""
    assert len(key) == len(state) == 16
    return fixed_xor(key, state)

def key_expansion(key : bytes, rounds : int = 10) -> List[bytes]:
    """Generate the Keys"""
    assert len(key) == 16
    words = [key[i:i + 4] for i in range(0, 16, 4)]

    for round in range(1, rounds + 1):
        n = (round - 1) * 4
        word = words[n + 3]
        word_prime = (fixed_xor(sub_word(rot_word(word)), words[n]))
        words.append(fixed_xor(word_prime, rcon(round)))
        for Nround in range(1, 4):
            words.append(fixed_xor(words[n + Nround], words[-1]))
    keys = [b"".join(words[i*4 : i*4 + 4]) for i in range(rounds + 1)]
    return keys

def rcon(i: int) -> bytes:
    """Round constant for GF(2^8)"""
    return bytes([RCON[i]]) + bytes(3)

def fixed_xor(a: bytes, b: bytes) -> bytes:
    return bytes(i ^ j for (i, j) in zip(a, b))

def to_bytes(x: int) -> bytes:
    return x.to_bytes(max((x.bit_length() + 7) // 8, 1), "big")

def encrypt(key : bytes, message : bytes, rounds : int = 10) -> bytes:
    assert len(key) == len(message) == 16

    keys = key_expansion(key)

    state = fixed_xor(message, keys[0])
    for i in range(1, rounds):
        state = add_round_key(keys[i], mix_columns(shift_rows(sub_bytes(state))))
    state = add_round_key(keys[rounds], shift_rows(sub_bytes(state)))

    return state

def decrypt(key : bytes, cipher : bytes, rounds : int = 10) -> bytes:
    assert len(key) == len(cipher) == 16

    keys = key_expansion(key)

    state = add_round_key(keys[rounds], cipher)
    state = sub_bytes_inverse(shift_rows_inverse(state))

    for i in range(rounds - 1, 0, -1):
        state = add_round_key(keys[i], state)
        state = sub_bytes_inverse(shift_rows_inverse(mix_columns_inverse(state)))
    state = add_round_key(keys[0], state)

    return state

if __name__ == "__main__" :
    parser = ArgumentParser()
    parser.add_argument("-text", help="Plain/Cipher (Ciphertext in Hexa Pls)", type=str)
    parser.add_argument("-key", help="Key (16 bytes Length)", type=str)
    parser.add_argument("-rounds", help="Rounds number", type=int, default=10)
    parser.add_argument("--encrypt", help="To Encrypt a Plaintext", action="store_true")
    parser.add_argument("--decrypt", help="To Decrypt a Ciphertext", action="store_true")
    args = parser.parse_args()
    
    text, key, rounds = (str(args.text), str(args.key), args.rounds)
    if args.encrypt : 
        text, key = (bytes(text, "utf-8"), bytes(key, "utf-8"))
        plains = [text[16 * i: 16 * (i + 1)] for i in range((int(len(text)/16))+1)]
        while(len(plains[-1]) < 16): plains[-1] += b"\x00"
        print("Ciphertext : ")
        for plain in plains:
            ciphertext = encrypt(key, plain, rounds)
            print(ciphertext.hex())
    if args.decrypt : 
        text, key = (bytes.fromhex(text), bytes(key, "utf-8"))
        ciphers = [text[16 * i: 16 * (i + 1)] for i in range((int(len(text)/16))+1)]
        while(len(ciphers[-1]) < 16): ciphers[-1] += b"\x00"
        for cipher in ciphers:
            plaintext = decrypt(key, cipher, rounds)
            print("Plaintext : " + str(plaintext))
